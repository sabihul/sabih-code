...
if __name__ == '__main__':
    ...
    FinalData = pd.read_csv(Output_Path + "FinalData_Fr_Prediction.csv")
    ...
    Dummy_Var_Df = pd.get_dummies(FinalData[["Sub Segment", "Month", "Quarter_Num"]])
    ...
    ### Loading the Model:
    Model = pickle.load(open(Input_Path + "FinalModel_XGBoost_22092020.sav", 'rb'))

    # Selecting the data for Prediction:
    Pred_Months = ['2020 2', '2020 3', '2020 7', '2020 8']
    ...
    Final_Prediction_Df.to_csv(Output_Path + "Final_Prediction_Df_v2.csv", index=False)
